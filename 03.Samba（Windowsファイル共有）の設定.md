...
```bash
[root@centos8 ~]# cp /etc/samba/smb.conf /etc/samba/smb.conf.orig
```
...
```bash
[root@centos8 ~]# wget https://raw.githubusercontent.com/tomgoodsun/centos8_lamp_configs/master/config/etc/samba/smb.conf -O /etc/samba/smb.conf 
```
